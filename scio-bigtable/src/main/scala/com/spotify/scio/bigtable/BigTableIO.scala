    ...
    var read = beam.BigtableIO
      ...
      .withBigtableOptionsConfigurator(
        new SerializableFunction[BigtableOptions.Builder,
          ...
          override def apply(
            input: BigtableOptions.Builder): BigtableOptions.Builder =
            opts.toBuilder
        })
...
object BigtableRead {
  final case class ReadParam(keyRange: ByteKeyRange = null,
...
final case class BigtableWrite[T](bigtableOptions: BigtableOptions,
                                  tableId: String)(implicit ev: T <:< Mutation)
  ...
  override def read(sc: ScioContext,
                    params: ReadP): SCollection[(ByteString, Iterable[T])] =
    throw new IllegalStateException(
    ...
    val sink =
      params match {
        ...
        case BigtableWrite.Bulk(numOfShards, flushInterval) =>
          new BigtableBulkWriter(tableId,
                                 bigtableOptions,
                                 numOfShards,
                                 flushInterval)
      }
...
object BigtableWrite {
  ...
  final case class Bulk(numOfShards: Int,
