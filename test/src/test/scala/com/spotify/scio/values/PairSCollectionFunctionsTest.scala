...
class PairSCollectionFunctionsTest extends PipelineTest {
  ...
  "PairSCollection" should "support coGroup()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(("a", 1), ("b", 2), ("c", 3))
      val p2 = sc.parallelize(("a", 11L), ("b", 12L), ("d", 14L))
  ...
  it should "support coGroup() with duplicate keys" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(("a", 1), ("a", 2), ("b", 2), ("c", 3))
      val p2 = sc.parallelize(("a", 11L), ("b", 12L), ("b", 13L), ("d", 14L))
  ...
  it should "support 3-way coGroup()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(("a", 1), ("b", 2), ("c", 3))
      val p2 = sc.parallelize(("a", 11L), ("b", 12L), ("d", 14L))
      val p3 = sc.parallelize(("a", 21F), ("b", 22F), ("e", 25F))
  ...
  it should "support 4-way coGroup()" in {
    runWithContext { sc =>
      ...
      val p2 = sc.parallelize(("a", 11L), ("b", 12L), ("d", 14L))
      val p3 = sc.parallelize(("a", 21F), ("b", 22F), ("e", 25F))
      val p4 = sc.parallelize(("a", 31.0), ("b", 32.0), ("f", 36.0))
  ...
  it should "support fullOuterJoin()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(("a", 1), ("b", 2), ("c", 3))
      val p2 = sc.parallelize(("a", 11), ("b", 12), ("d", 14))
  ...
  it should "support fullOuterJoin() with duplicate keys" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(("a", 1), ("a", 2), ("b", 3), ("c", 4))
      val p2 = sc.parallelize(("a", 11), ("b", 12), ("b", 13), ("d", 14))
      val p = p1.fullOuterJoin(p2)
      p.internal should containInAnyOrder (
        ("a", (Some(1), Some(11))),
        ("a", (Some(2), Some(11))),
  ...
  it should "support aggregateByKey()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(1 to 100: _*).map(("a", _))
      val p2 = sc.parallelize(1 to 10: _*).map(("b", _))
  ...
  it should "support approxQuantilesByKey()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(0 to 100: _*).map(("a", _))
      val p2 = sc.parallelize(0 to 10: _*).map(("b", _))
  ...
  it should "support countApproxDistinctByKey()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 11), ("a", 12), ("b", 21), ("b", 22), ("b", 23))
  ...
  it should "support countByKey()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 11), ("a", 12), ("b", 21), ("b", 22), ("b", 23)).countByKey()
  ...
  it should "support flatMapValues()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 2)).flatMapValues(v => Seq(v + 10.0, v + 20.0))
  ...
  it should "support groupByKey()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("a", 10), ("b", 2), ("b", 20)).groupByKey().mapValues(_.toSet)
  ...
  it should "support keys()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 2), ("c", 3)).keys
  ...
  it should "support mapValues()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 2)).mapValues(_ + 10.0)
  ...
  it should "support reduceByKey()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 1), ("b", 2), ("c", 1), ("c", 2), ("c", 3)).reduceByKey(_ + _)
  ...
  it should "support sampleByKey()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 2), ("b", 2), ("c", 3), ("c", 3), ("c", 3)).sampleByKey(1)
  ...
  it should "support subtractByKey()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(("a", 1), ("b", 2), ("b", 3), ("c", 4), ("c", 5), ("c", 6))
      val p2 = sc.parallelize(("a", 10L), ("b", 20L))
  ...
  it should "support swap()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 2), ("c", 3)).swap
  ...
  it should "support topByKey()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 11), ("b", 12), ("c", 21), ("c", 22), ("c", 23))
      val r1 = p.topByKey(1)
  ...
  it should "support values()" in {
    runWithContext { sc =>
      val p = sc.parallelize(("a", 1), ("b", 2), ("c", 3)).values
  ...
  it should "support hashJoin() with duplicate keys" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(("a", 1), ("a", 2), ("b", 3))
      val p2 = sc.parallelize(("a", 11), ("b", 12), ("b", 13))
      val p = p1.hashJoin(p2)
