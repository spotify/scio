...
trait OverrideTypeProvider {
  ...
  /**
   * Returns a `context.Tree` representing a new instance of whatever type is appropriate. This is
   * called at macro expansion time.
   */
  def createInstance(c: blackbox.Context)(tpe: c.Type, tree: c.Tree): c.Tree

  /**
   * Called when we need to initialize the `toTable` method using annotations. This is called at
  ...
  /** Returns the `String` representation of the BigQuery column type. This is called at runtime. */
