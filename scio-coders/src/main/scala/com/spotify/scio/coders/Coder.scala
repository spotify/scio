...
final case class Transform[A, B] private (c: Coder[A], f: BCoder[A] => Coder[B])
    extends Coder[B]
final case class Disjunction[T, Id] private (idCoder: Coder[Id],
                                             id: T => Id,
                                             coder: Map[Id, Coder[T]])
    extends Coder[T]
final case class Record[T] private (cs: Array[(String, Coder[T])])
    extends Coder[Array[T]]
// KV are special in beam and need to be serialized using an instance of KvCoder.
final case class KVCoder[K, V] private (koder: Coder[K], voder: Coder[V])
...
private class RecordCoder[T: ClassTag](cs: Array[(String, BCoder[T])])
  ...
  def encode(value: Array[T], os: OutputStream): Unit = {
    ...
    while (i < value.length) {
      ...
      val v = value(i)
      onErrorMsg(
        s"Exception while trying to `encode` field ${label} with value ${v}") {
        c.encode(v, os)
      }
      i = i + 1
    }
...
sealed trait CoderGrammar {
  ...
  def transform[A, B](c: Coder[A])(f: BCoder[A] => Coder[B]): Coder[B] =
    Transform(c, f)
  def disjunction[T, Id: Coder](coder: Map[Id, Coder[T]])(
    id: T => Id): Coder[T] =
  ...
  private[scio] def sequence[T](
    cs: Array[(String, Coder[T])]): Coder[Array[T]] =
    Record(cs)
}
