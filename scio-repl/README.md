...
To create a ScioContext for Google Cloud Dataflow service, pass Dataflow pipeline options on
 REPL startup - `:newScio` will use those arguments when creating new Scio contexts. For example:
...
In this case we are reading data from GCS and performing computation in Dataflow service. Last line
 is an example of reading data from GCS file/tap.
...
scio> val forcast = tornadoes.flatMap(r => if (r.getBoolean("tornado")) Seq(r.getInt("month")) else Nil).countByValue().map(kv => TableRow("month" -> kv._1, "tornado_count" -> kv._2))
...
In this example we combine power of BigQuery and flexibility of Dataflow. We first query BigQuery
 ...
 tornadoes. We take (`take(3)`) some data back locally (`materialize`) to check the results.
...
res1: com.google.cloud.dataflow.sdk.PipelineResult.State = RUNNING
```
...
Keep in mind tho that currently `BlockingDataflowPipelineRunner` is still pretty noisy on stdout,
...
Whenever possible leverage BigQuery! `@BigQueryType` annotations enable type safe and civilized
 ...
 BigQuery client to read and write typed data directly without Scio context.
