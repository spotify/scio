...
class SCollectionTest extends PipelineTest {
  ...
  "SCollection" should "support setName()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1, 2, 3, 4, 5).setName("MySCollection")
  ...
  it should "support ++ operator" in {
    runWithContext { sc =>
      val p1 = sc.parallelize("a", "b", "c")
      val p2 = sc.parallelize("d", "e", "f")
      val r1 = p1 ++ p2
      val r2 = p1.union(p2)
  ...
  it should "support ++ operator with duplicates" in {
    runWithContext { sc =>
      val p1 = sc.parallelize("a", "b", "c", "a", "d")
      val p2 = sc.parallelize("d", "e", "f", "a", "d")
      val r1 = p1 ++ p2
      val r2 = p1.union(p2)
  ...
  it should "support intersection()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(1, 2, 3, 4, 5)
      val p2 = sc.parallelize(2, 4, 6, 8, 10)
  ...
  it should "support intersection() with duplicates" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(1, 2, 3, 4, 5, 2, 4)
      val p2 = sc.parallelize(2, 4, 6, 8, 10, 2, 4)
      val p = p1.intersection(p2)
      p.internal should containInAnyOrder (2, 4)
    }
  }
  ...
  it should "support partition()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1, 2, 3, 4, 5, 6).partition(2, _ % 2)
  ...
  it should "support aggregate()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1 to 100: _*)
  ...
  it should "support combine()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1 to 100: _*).combine(_.toDouble)(_ + _)(_ + _)
  ...
  it should "support count()" in {
    runWithContext { sc =>
      val p = sc.parallelize("a", "b", "c").count()
  ...
  it should "support countApproxDistinct()" in {
    runWithContext { sc =>
      val p = sc.parallelize("a", "b", "b", "c", "c", "c")
  ...
  it should "support countByValue()" in {
    runWithContext { sc =>
      val p = sc.parallelize("a", "b", "b", "c", "c", "c").countByValue()
  ...
  it should "support distinct()" in {
    runWithContext { sc =>
      val p = sc.parallelize("a", "b", "b", "c", "c", "c").distinct()
  ...
  it should "support filter()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1, 2, 3, 4, 5).filter(_ % 2 == 0)
      p.internal should containInAnyOrder (2, 4)
    }
  }
  ...
  it should "support flatMap()" in {
    runWithContext { sc =>
      val p = sc.parallelize("a b c", "d e", "f").flatMap(_.split(" "))
  ...
  it should "support groupBy()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1, 2, 3, 4).groupBy(_ % 2).mapValues(_.toSet)
  ...
  it should "support keyBy()" in {
    runWithContext { sc =>
      val p = sc.parallelize("hello", "world").keyBy(_.substring(0, 1))
  ...
  it should "support map()" in {
    runWithContext { sc =>
      val p = sc.parallelize("1", "2", "3").map(_.toInt)
  ...
  it should "support max()" in {
    runWithContext { sc =>
      def max[T: ClassTag : Numeric](elems: T*) = sc.parallelize(elems: _*).max
  ...
  it should "support mean()" in {
    runWithContext { sc =>
      def mean[T: ClassTag : Numeric](elems: T*) = sc.parallelize(elems: _*).mean
  ...
  it should "support min()" in {
    runWithContext { sc =>
      def min[T: ClassTag : Numeric](elems: T*) = sc.parallelize(elems: _*).min
  ...
  it should "support quantilesApprox()" in {
    runWithContext { sc =>
      val p = sc.parallelize(0 to 100: _*).quantilesApprox(5)
  ...
  it should "support randomSplit()" in {
    runWithContext { sc =>
      ...
      val p1 = sc.parallelize(0 to 1000: _*).randomSplit(Array(0.3, 0.7))
      val p2 = sc.parallelize(0 to 1000: _*).randomSplit(Array(0.2, 0.3, 0.5))
  ...
  it should "support reduce()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1, 2, 3, 4, 5).reduce(_ + _)
  ...
  it should "support sample()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1, 1, 1, 1, 1)
      val r1 = p.sample(1)
      val r2 = p.sample(5)
  ...
  it should "support subtract() with duplicates" in {
    runWithContext { sc =>
      val p1 = sc.parallelize(1, 2, 3, 4, 5, 1, 3, 5)
      val p2 = sc.parallelize(2, 4, 6, 8, 10)
      val p = p1.subtract(p2)
      p.internal should containInAnyOrder (1, 3, 5)
    }
  ...
  it should "support sum()" in {
    runWithContext { sc =>
      def sum[T: ClassTag : Semigroup](elems: T*) = sc.parallelize(elems: _*).sum
  ...
  it should "support take()" in {
    runWithContext { sc =>
      val p = sc.parallelize(1, 2, 3, 4, 5)
  ...
  it should "support cross()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize("a", "b", "c")
      val p2 = sc.parallelize(1)
      val p3 = sc.parallelize(1, 2)
      val s1 = p1.cross(p2)
      val s2 = p1.cross(p3)
  ...
  it should "support hashLookup()" in {
    runWithContext { sc =>
      val p1 = sc.parallelize("a", "b", "c")
      val p2 = sc.parallelize(("a", 1), ("b", 2), ("b", 3))
