...
class SCollectionWithSideInputTest extends PipelineTest {
  ...
  "SCollectionWithSideInput" should "support filter()" in {
    runWithContext { pipeline =>
      ...
      val s3 = p1.withKVSideInput(p4).filter((x, s) => (x + s(x).sum) % 2 == 0).toSCollection
  ...
  it should "support flatMap()" in {
    runWithContext { pipeline =>
      ...
      val s3 = p1.withKVSideInput(p4).flatMap((x, s) => s.getOrElse(x, Seq()).map(x + _)).toSCollection
  ...
  it should "support keyBy()" in {
    runWithContext { pipeline =>
      ...
      val s3 = p1.withKVSideInput(p4).keyBy((x, s) => x + s(x).sum).toSCollection
  ...
  it should "support map()" in {
    runWithContext { pipeline =>
      ...
      val s3 = p1.withKVSideInput(p4).map((x, s) => x + s(x).sum).toSCollection
