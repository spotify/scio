...
object JdbcIO {
  ...
  private[jdbc] def jdbcIoId(opts: JdbcConnectionOptions,
                             query: String): String = {
...
final case class JdbcSelect[T: ClassTag](readOptions: JdbcReadOptions[T])
    extends ScioIO[T] {
  ...
  override def read(sc: ScioContext, params: ReadP): SCollection[T] = {
    ...
    var transform = beam.JdbcIO
      ...
      .withDataSourceConfiguration(
        getDataSourceConfig(readOptions.connectionOptions))
...
final case class JdbcWrite[T](writeOptions: JdbcWriteOptions[T])
    extends ScioIO[T] {
  ...
  override def write(data: SCollection[T], params: WriteP): Future[Tap[T]] = {
    var transform = beam.JdbcIO
      .write[T]()
      .withDataSourceConfiguration(
        getDataSourceConfig(writeOptions.connectionOptions))
      .withStatement(writeOptions.statement)
    if (writeOptions.preparedStatementSetter != null) {
      transform = transform
        .withPreparedStatementSetter(
          new beam.JdbcIO.PreparedStatementSetter[T] {
            override def setParameters(
              element: T,
              preparedStatement: PreparedStatement): Unit =
