...
object FixTfParameter {
  ...
  val result: SCollection[B] = elements.predict[B, C]("gs://model-path", fetchOpts, options)(toTensors)(fromTensors)
  val b: SCollection[B] = elements.predictWithSigDef[B, C]("gs://model-path", options)(toTensors)(fromTensors _)
}
