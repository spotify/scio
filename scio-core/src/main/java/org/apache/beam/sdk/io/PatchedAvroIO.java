...
public class PatchedAvroIO {
  ...
  public static <T> Read<T> read(Class<T> recordClass) {
    return new AutoValue_PatchedAvroIO_Read.Builder<T>()
  ...
  public static Read<GenericRecord> readGenericRecords(Schema schema) {
    return new AutoValue_PatchedAvroIO_Read.Builder<GenericRecord>()
  ...
  public static <T> Write<T> write(Class<T> recordClass) {
    return PatchedAvroIO.<T>defaultWriteBuilder()
        .setRecordClass(recordClass)
        .setSchema(ReflectData.get().getSchema(recordClass))
        .build();
  }
  ...
  public static Write<GenericRecord> writeGenericRecords(Schema schema) {
    return PatchedAvroIO.<GenericRecord>defaultWriteBuilder()
        .setRecordClass(GenericRecord.class)
        .setSchema(schema)
        .build();
  }
  ...
  private static <T> Write.Builder<T> defaultWriteBuilder() {
    return new AutoValue_PatchedAvroIO_Write.Builder<T>()
      ...
      Bounded<T> read =
          getRecordClass() == GenericRecord.class
              ? (Bounded<T>) org.apache.beam.sdk.io.Read.from(
                  PatchedAvroSource.from(getFilepattern()).withSchema(getSchema()))
              : org.apache.beam.sdk.io.Read.from(
                  PatchedAvroSource.from(getFilepattern()).withSchema(getRecordClass()));
