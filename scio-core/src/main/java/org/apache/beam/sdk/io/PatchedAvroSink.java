...
class PatchedAvroSink<T> extends FileBasedSink<T> {
  ...
  public WriteOperation<T> createWriteOperation() {
    return new PatchedAvroWriteOperation<>(this, coder, codec, metadata);
  }

  /** A {@link WriteOperation WriteOperation} for Avro files. */
  private static class PatchedAvroWriteOperation<T> extends WriteOperation<T> {
    private final AvroCoder<T> coder;
    private final SerializableAvroCodecFactory codec;
    private final ImmutableMap<String, Object> metadata;

    private PatchedAvroWriteOperation(PatchedAvroSink<T> sink,
                               AvroCoder<T> coder,
                               SerializableAvroCodecFactory codec,
                               ImmutableMap<String, Object> metadata) {
      super(sink);
    ...
    public Writer<T> createWriter() throws Exception {
      return new PatchedAvroWriter<>(this, coder, codec, metadata);
    }
  }

  /** A {@link Writer Writer} for Avro files. */
  private static class PatchedAvroWriter<T> extends Writer<T> {
    private final AvroCoder<T> coder;
    private DataFileWriter<T> dataFileWriter;
    private SerializableAvroCodecFactory codec;
    private final ImmutableMap<String, Object> metadata;

    public PatchedAvroWriter(WriteOperation<T> writeOperation,
                      AvroCoder<T> coder,
                      SerializableAvroCodecFactory codec,
                      ImmutableMap<String, Object> metadata) {
