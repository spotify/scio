...
/**
 * A kryo {@link Serializer} for unmodifiable sets created via {@link
...
public class UnmodifiableSetSerializer extends UnmodifiableJavaCollectionSerializer<Set<?>> {
  ...
  public static IKryoRegistrar registrar() {
    return new SingleRegistrar(
        Collections.unmodifiableSet(Collections.EMPTY_SET).getClass(),
        new UnmodifiableSetSerializer());
  }

  @Override
  protected Field getInnerField() throws Exception {
    return Class.forName("java.util.Collections$UnmodifiableSet")
        .getSuperclass()
        .getDeclaredField("c");
  }
