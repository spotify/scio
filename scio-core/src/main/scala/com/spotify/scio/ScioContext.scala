...
import com.twitter.algebird.{Max, SparseVector}
...
class ScioContext private[scio] (val options: PipelineOptions,
  ...
  : SCollection[(Double, SparseVector[Double])] = {
    val data = this.textFile(path)
      .map(_.trim)
      .filter(line => !(line.isEmpty || line.startsWith("#")))
      .map(parseLibSVMRecord)

    val featureCntCol = if(numFeatures.isDefined){
      this.parallelize(List(numFeatures.get))
    } else {
    ...
    data.cross(featureCntCol).map{case((label, indicies, values), featureCount) =>
      val data = indicies.zip(values).toMap
      (label, SparseVector[Double](data, 0.0, featureCount))
    }
  }
