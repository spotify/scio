...
object ScalableBloomFilter extends ApproxFilterCompanion {
  ...
  override implicit def filterCoder[T: Hash]: Coder[Filter[T]] = {
    Coder.xmap(Coder[MutableScalableBloomFilter[T]])(a => new ScalableBloomFilter[T](a), b => b.impl)
  }

  override protected def createImpl[T: Funnel](elems: Iterable[T], expectedInsertions: Long, fpp: Double): Filter[T] = {
    new ScalableBloomFilter(MutableScalableBloomFilter(expectedInsertions, fpp) ++= elems)
  }

  override private[hash] def partitionSettings(expectedInsertions: Long, fpp: Double, maxBytes: Int): PartitionSettings = {
    val defaultPartitionSettings = BloomFilter.partitionSettings(expectedInsertions, fpp, maxBytes)
    PartitionSettings(defaultPartitionSettings.partitions, defaultPartitionSettings.expectedInsertions, defaultPartitionSettings.sizeBytes)
  }
}