  ...
  def tsqlImpl[B: c.WeakTypeTag](
    ...
    distinctSCollections.values.toList match {
      case (c0, t0) :: Nil =>
        ...
        val (implB, _) = SchemaMacroHelpers.inferImplicitSchema(c)(needB)
        val (implA, _) = SchemaMacroHelpers.inferImplicitSchema(c)(t0)
      ...
      case (c0, t0) :: (c1, t1) :: Nil =>
        ...
        val (implB, _) = SchemaMacroHelpers.inferImplicitSchema(c)(needB)
        val (implA, _) = SchemaMacroHelpers.inferImplicitSchema(c)(t0)
        val (implC, _) = SchemaMacroHelpers.inferImplicitSchema(c)(t1)
