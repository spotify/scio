...
package object transforms {
  ...
  implicit class CustomParallelismSCollection[T](val self: SCollection[T]) {
    private def parallelCollectFn[U](parallelism: Int)(
      pfn: PartialFunction[T, U]): DoFn[T, U] =
    ...
    private def parallelFilterFn(parallelism: Int)(
      f: T => Boolean): DoFn[T, T] =
    ...
    private def parallelFlatMapFn[U](parallelism: Int)(
      f: T => TraversableOnce[U]): DoFn[T, U] =
     ...
     * @group transform
     */
    def mapWithParallelism[U: Coder](parallelism: Int)(
      fn: T => U): SCollection[U] =
  ...
  implicit class PipeSCollection(val self: SCollection[String]) extends AnyVal {
    ...
    def pipe(command: String,
      ...
      self.applyTransform(
        ParDo.of(new PipeDoFn(command, env, dir, sCmds, tCmds)))
    }
    ...
    def pipe(cmdArray: Array[String],
      ...
      val tCmds = if (teardownCmds == null) null else teardownCmds.asJava
      self.applyTransform(
        ParDo.of(new PipeDoFn(cmdArray, env, dir, sCmds, tCmds)))
    }
  ...
  implicit class RichAsyncLookupDoFnTry[A](val self: AsyncLookupDoFn.Try[A])
      extends AnyVal {
