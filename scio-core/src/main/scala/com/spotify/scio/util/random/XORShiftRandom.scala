 ...
 *   3.5 times faster than [[java.util.Random]], partly because of the algorithm, but also due to
 *   renouncing thread safety. JDK's implementation uses an AtomicLong seed, this class uses a
 *   regular Long. We can forgo thread safety since we use a new instance of the RNG for each
 *   thread.
 */
