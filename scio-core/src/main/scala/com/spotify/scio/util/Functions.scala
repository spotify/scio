...
private[scio] object Functions {
  ...
  // TODO: rename
  private abstract class KryoCombineFn[VI, VA, VO]
    ...
    override def getAccumulatorCoder(registry: CoderRegistry,
                                     inputCoder: BCoder[VI]): BCoder[VA] =
  ...
  def aggregateFn[T: Coder, U: Coder](zeroValue: U)(
    seqOp: (U, T) => U,
    combOp: (U, U) => U): CombineFn[T, (U, JList[T]), U] =
    new KryoCombineFn[T, (U, JList[T]), U] {
      ...
      override def addInput(accumulator: (U, JList[T]),
                            input: T): (U, JList[T]) = {
        val (a, l) = accumulator
        l.add(input)
      ...
      override def mergeAccumulators(
        accumulators: JIterable[(U, JList[T])]): (U, JList[T]) = {
  ...
  def combineFn[T: Coder, C: Coder](
    ...
    new KryoCombineFn[T, (Option[C], JList[T]), C] {
      ...
      override def addInput(accumulator: (Option[C], JList[T]),
                            input: T): (Option[C], JList[T]) = {
        val (a, l) = accumulator
        l.add(input)
        if (l.size() >= BUFFER_SIZE) {
  ...
  private abstract class ReduceFn[T: Coder]
      extends KryoCombineFn[T, JList[T], T] {
    ...
    override def mergeAccumulators(
      accumulators: JIterable[JList[T]]): JList[T] = {
