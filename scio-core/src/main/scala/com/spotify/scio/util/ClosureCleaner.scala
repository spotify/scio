...
private[scio] object ClosureCleaner {
  ...
  final def apply[T <: AnyRef](func: T): T = {
    try {
      ...
      case e: IllegalArgumentException
          if e.getCause.isInstanceOf[NotSerializableException] =>
        clean(func)
  ...
  def outerClassesOf(obj: AnyRef): List[(Class[_], AnyRef)] = {
    @tailrec
    def loop(obj: AnyRef,
             hierarchy: List[(Class[_], AnyRef)]): List[(Class[_], AnyRef)] = {
 ...
 */
private final class TransitiveClosureCleaner(val func: AnyRef)
    extends ClosureCleaner {
  import ClosureCleaner._
...
private final case class MethodIdentifier[T](cls: Class[T],
                                             name: String,
                                             desc: String)
...
private final class InnerClosureFinder(output: MSet[Class[_]])
    extends ClassVisitor(ASM6) {
