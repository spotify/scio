...
class PairHashSCollectionFunctions[K, V](val self: SCollection[(K, V)]) {
  ...
  def hashJoin[W: Coder](that: SCollection[(K, W)])(
    ...
    voder: Coder[V]): SCollection[(K, (V, W))] = self.transform { in =>
    ...
    self.transform { in =>
      ...
      val leftHashed = in
        ...
        .flatMap { (kv, s) =>
          ...
          val m = s(side)
          if (m.contains(k)) {
            m(k).iterator.map[(K, (Option[V], Option[W]), Boolean)](w =>
              (k, (Some(v), Some(w)), true))
          } else {
      ...
      val rightHashed = leftHashed
        ...
        .flatMap { (mk, s) =>
          ...
          (m.keySet diff mk.toSet)
            .flatMap(k =>
              m(k).iterator.map[(K, (Option[V], Option[W]))](w =>
                (k, (None, Some(w)))))
        }
        .toSCollection
