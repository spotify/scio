...
class SCollectionWithSideInput[T: Coder] private[values] (
  ...
  def filter(
    f: (T, SideInputContext[T]) => Boolean): SCollectionWithSideInput[T] = {
    val o = this
  ...
  def keyBy[K: Coder](
    f: (T, SideInputContext[T]) => K): SCollectionWithSideInput[(K, T)] =
    this.map((x, s) => (f(x, s), x))

  /** [[SCollection.map]] with an additional [[SideInputContext]] argument. */
  def map[U: Coder](
    f: (T, SideInputContext[T]) => U): SCollectionWithSideInput[U] = {
    val o = this
  ...
  private[values] def transformWithSideOutputs(sideOutputs: Seq[SideOutput[T]],
    ...
    val sideTags = TupleTagList.of(
      sideOutputs.map(e => e.tupleTag.asInstanceOf[TupleTag[_]]).asJava)
