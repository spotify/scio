...
object SCollection {
  ...
  implicit def makeDoubleSCollectionFunctions(
    s: SCollection[Double]): DoubleSCollectionFunctions =
...
sealed trait SCollection[T] extends PCollectionWrapper[T] {
  ...
  private[scio] def withState(
    f: SCollection.State => SCollection.State): SCollection[T] = {
    _state = f(_state)
    this
  }
  ...
  /** Apply a transform. */
  private[scio] def transform[U](
    f: SCollection[T] => SCollection[U]): SCollection[U] = {
    val o = internal.apply(
      this.tfName,
   ...
   */
  def withFanout(fanout: Int)(
    implicit coder: Coder[T]): SCollectionWithFanout[T] =
   ...
   * @group collection
   */
  def intersection(that: SCollection[T])(
  ...
  def partition(numPartitions: Int, f: T => Int): Seq[SCollection[T]] =
    this
      .applyInternal(
        Partition.of[T](numPartitions,
                        Functions.partitionFn[T](numPartitions, f)))
      .getAll
      .asScala
  ...
  def aggregate[U: Coder](zeroValue: U)(
    ...
    this.pApply(
      Combine.globally(Functions.aggregateFn(zeroValue)(seqOp, combOp)))
   ...
   * @group transform
   */
  def countApproxDistinct(
    maximumEstimationError: Double = 0.02): SCollection[Long] =
    this
    ...
    this.pApply(
      Distinct
        ...
        .withRepresentativeType(
          TypeDescriptor.of(ctu.runtimeClass).asInstanceOf[TypeDescriptor[U]]))
  ...
  def filter(f: T => Boolean): SCollection[T] =
    this.pApply(
      Filter.by(Functions.serializableFn(f.asInstanceOf[T => JBoolean])))
  ...
  def max(implicit ord: Ordering[T],
          coder: Coder[T],
          dummy: DummyImplicit): SCollection[T] =
    max(ord)(coder)
   ...
   * @group transform
   */
  def mean(implicit ev: Numeric[T]): SCollection[Double] = this.transform {
    in =>
   ...
   * @group hash
   */
  def cross[U: Coder](that: SCollection[U])(
    implicit tcoder: Coder[T]): SCollection[(T, U)] =
    this.transform { in =>
  ...
  def hashLookup[V: Coder](that: SCollection[(T, V)])(
    implicit coder: Coder[T]): SCollection[(T, Iterable[V])] = this.transform {
    in =>
  ...
  def asSingletonSideInput(defaultValue: T): SideInput[T] =
    new SingletonSideInput[T](
      this.applyInternal(View.asSingleton().withDefaultValue(defaultValue)))
   ...
   * @group side
   */
  def withSideInputs(sides: SideInput[_]*)(
    implicit coder: Coder[T]): SCollectionWithSideInput[T] =
    ...
    if (options.accumulationMode != null) {
      if (options.accumulationMode == AccumulationMode.ACCUMULATING_FIRED_PANES) {
        ...
        throw new RuntimeException(
          s"Unsupported accumulation mode ${options.accumulationMode}")
      }
    }
   ...
   */
  def withPaneInfo(
    implicit coder: Coder[(T, PaneInfo)]): SCollection[(T, PaneInfo)] =
    this.parDo(new DoFn[T, (T, PaneInfo)] {
      @ProcessElement
      private[scio] def processElement(
        c: DoFn[T, (T, PaneInfo)]#ProcessContext): Unit =
   ...
   */
  def withTimestamp(
    implicit coder: Coder[(T, Instant)]): SCollection[(T, Instant)] =
    this.parDo(new DoFn[T, (T, Instant)] {
      @ProcessElement
      private[scio] def processElement(
        c: DoFn[T, (T, Instant)]#ProcessContext): Unit =
  ...
  def withWindow[W <: BoundedWindow](
    implicit tcoder: Coder[T]): SCollection[(T, W)] =
    this
   ...
   * @group window
   */
  def timestampBy(f: T => Instant,
                  allowedTimestampSkew: Duration = Duration.ZERO)(
    implicit coder: Coder[T]): SCollection[T] =
    this.applyTransform(
      WithTimestamps
  ...
  def readAllBytes(implicit ev: T <:< String): SCollection[Array[Byte]] =
    if (context.isTest) {
      ...
      this
        .asInstanceOf[SCollection[String]]
        .applyTransform(
          new PTransform[PCollection[String], PCollection[Array[Byte]]]() {
            override def expand(
              input: PCollection[String]): PCollection[Array[Byte]] =
              input
   ...
   * @group output
   */
  def saveAsDatastore(projectId: String)(
    implicit ev: T <:< Entity): Future[Tap[Entity]] =
  ...
  private[scio] def saveAsInMemoryTap(
    implicit coder: Coder[T]): Future[Tap[T]] = {
   ...
   */
  def write(io: ScioIO[T])(params: io.WriteP)(
    implicit coder: Coder[T]): Future[Tap[T]] =
    writeImpl(io)(params)
