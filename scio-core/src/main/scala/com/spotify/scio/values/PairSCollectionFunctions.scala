...
private object PairSCollectionFunctions {
  ...
  def optimalBFSettings(numEntries: Long, fpProb: Double): BFSettings = {
    ...
    logger.info(
      s"Optimal Bloom Filter settings for numEntries = $numEntries, fpProb = $fpProb")
    logger.info(
...
class PairSCollectionFunctions[K, V](val self: SCollection[(K, V)]) {
  ...
  private[scio] def toKV(implicit koder: Coder[K],
  ...
  private[values] def applyPerKey[UI: Coder, UO: Coder](
    ...
    f: KV[K, UI] => (K, UO))(implicit koder: Coder[K],
                             voder: Coder[V]): SCollection[(K, UO)] = {
    val o = self.applyInternal(
      new PTransform[PCollection[(K, V)], PCollection[(K, UO)]](null) {
        override def expand(
          input: PCollection[(K, V)]): PCollection[(K, UO)] = {
          var kv = input
          ...
          kv.apply("KvToTuple",
                   ParDo.of(Functions.mapFn[KV[K, UI], (K, UO)](f)))
   ...
   */
  def applyPerKeyDoFn[U: Coder](t: DoFn[KV[K, V], KV[K, U]])(
    implicit koder: Coder[K],
    vcoder: Coder[V]): SCollection[(K, U)] =
    this.applyPerKey(
      ParDo
        .of(t)
  ...
  def withHotKeyFanout(hotKeyFanout: K => Int)(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollectionWithHotKeyFanout[K, V] =
  ...
  def cogroup[W: Coder](that: SCollection[(K, W)])(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[(K, (Iterable[V], Iterable[W]))] =
  ...
  def cogroup[W1: Coder, W2: Coder](
    ...
    that2: SCollection[(K, W2)])(implicit koder: Coder[K], voder: Coder[V])
   ...
   * @group cogroup
   */
  def cogroup[W1: Coder, W2: Coder, W3: Coder](that1: SCollection[(K, W1)],
                                               that2: SCollection[(K, W2)],
                                               that3: SCollection[(K, W3)])(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[
    (K, (Iterable[V], Iterable[W1], Iterable[W2], Iterable[W3]))] =
   ...
   * @group cogroup
   */
  def groupWith[W1: Coder, W2: Coder, W3: Coder](that1: SCollection[(K, W1)],
                                                 that2: SCollection[(K, W2)],
                                                 that3: SCollection[(K, W3)])(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[
  ...
  protected def sparseOuterJoinImpl[W: Coder](that: SCollection[(K, W)],
    ...
    val unique = partitionedSelf(lhsUnique).map(kv =>
      (kv._1, (Option(kv._2), Option.empty[W])))
  ...
  def aggregateByKey[A: Coder, U: Coder](aggregator: Aggregator[V, A, U])(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[(K, U)] = self.transform { in =>
  ...
  def approxQuantilesByKey(numQuantiles: Int, ord: Ordering[V])(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[(K, Iterable[V])] =
    this.applyPerKey(ApproximateQuantiles.perKey(numQuantiles, ord),
                     kvListToTuple[K, V])
  ...
  def combineByKey[C: Coder](createCombiner: V => C)(mergeValue: (C, V) => C)(
    mergeCombiners: (C, C) => C)(implicit koder: Coder[K],
                                 voder: Coder[V]): SCollection[(K, C)] =
    this.applyPerKey(
      Combine.perKey(
  ...
  def countApproxDistinctByKey(sampleSize: Int)(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[(K, Long)] =
  ...
  def countApproxDistinctByKey(maximumEstimationError: Double = 0.02)(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[(K, Long)] =
    this.applyPerKey(ApproximateUnique.perKey[K, V](maximumEstimationError),
                     klToTuple[K])
   ...
   * @group per_key
   */
  def countByKey(implicit koder: Coder[K],
                 voder: Coder[V]): SCollection[(K, Long)] =
  ...
  def foldByKey(zeroValue: V)(op: (V, V) => V)(
    ...
    this.applyPerKey(Combine.perKey(Functions.aggregateFn(zeroValue)(op, op)),
                     kvToTuple[K, V])
   ...
   * @group per_key
   */
  def foldByKey(implicit mon: Monoid[V],
                koder: Coder[K],
                voder: Coder[V]): SCollection[(K, V)] =
   ...
   * @group per_key
   */
  def groupByKey(implicit koder: Coder[K],
                 voder: Coder[V]): SCollection[(K, Iterable[V])] =
  ...
  def intersectByKey(that: SCollection[K])(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[(K, V)] = self.transform {
  ...
  def maxByKey(ord: Ordering[V])(implicit koder: Coder[K],
                                 voder: Coder[V]): SCollection[(K, V)] =
  ...
  def minByKey(implicit koder: Coder[K],
               voder: Coder[V],
               ord: Ordering[V]): SCollection[(K, V)] =
    this.reduceByKey(ord.min)

  def minByKey(ord: Ordering[V])(implicit koder: Coder[K],
                                 voder: Coder[V]): SCollection[(K, V)] =
   ...
   * @group per_key
   */
  def reduceByKey(op: (V, V) => V)(implicit koder: Coder[K],
                                   voder: Coder[V]): SCollection[(K, V)] =
  ...
  def sampleByKey(sampleSize: Int)(
    implicit koder: Coder[K],
    voder: Coder[V]): SCollection[(K, Iterable[V])] =
    this.applyPerKey(Sample.fixedSizePerKey[K, V](sampleSize),
                     kvIterableToTuple[K, V])
   ...
   * @group per_key
   */
  def sumByKey(implicit sg: Semigroup[V],
               koder: Coder[K],
               voder: Coder[V]): SCollection[(K, V)] =
  ...
  def sumByKey(sg: Semigroup[V])(implicit koder: Coder[K],
                                 voder: Coder[V],
                                 d: DummyImplicit): SCollection[(K, V)] =
   ...
   * @group per_key
   */
  def topByKey(num: Int)(implicit ord: Ordering[V],
                         koder: Coder[K],
                         voder: Coder[V],
                         dummy: DummyImplicit): SCollection[(K, Iterable[V])] =
   ...
   * @group transform
   */
  def flattenValues[U: Coder](implicit ev: V <:< TraversableOnce[U],
                              koder: Coder[K]): SCollection[(K, U)] =
   ...
   */
  def asMapSideInput(implicit koder: Coder[K],
                     voder: Coder[V]): SideInput[Map[K, V]] = {
    val o = self.applyInternal(
      new PTransform[PCollection[(K, V)], PCollectionView[JMap[K, V]]]() {
        override def expand(
          input: PCollection[(K, V)]): PCollectionView[JMap[K, V]] =
          input
            .apply(toKvTransform)
            .setCoder(CoderMaterializer.kvCoder[K, V](context))
            .apply(View.asMap())
   ...
   * Currently, the resulting map is required to fit into memory.
   */
  def asMultiMapSideInput(implicit koder: Coder[K],
                          voder: Coder[V]): SideInput[Map[K, Iterable[V]]] = {
    val o = self.applyInternal(
      new PTransform[PCollection[(K, V)],
                     PCollectionView[JMap[K, JIterable[V]]]]() {
        override def expand(
          input: PCollection[(K, V)]): PCollectionView[JMap[K, JIterable[V]]] =
          input
            .apply(toKvTransform)
            .setCoder(CoderMaterializer.kvCoder[K, V](context))
