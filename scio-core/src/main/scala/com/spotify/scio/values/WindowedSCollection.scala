...
case class WindowedValue[T](value: T,
                            timestamp: Instant,
...
class WindowedSCollection[T: Coder] private[values] (
  ...
  def filter(f: WindowedValue[T] => Boolean): WindowedSCollection[T] =
    new WindowedSCollection(
      this.parDo(FunctionsWithWindowedValue.filterFn(f)).internal,
      context)
  ...
  def flatMap[U: Coder](
    ...
    new WindowedSCollection(
      this.parDo(FunctionsWithWindowedValue.flatMapFn(f)).internal,
      context)
  ...
  def map[U: Coder](
    f: WindowedValue[T] => WindowedValue[U]): WindowedSCollection[U] =
    new WindowedSCollection(
      this.parDo(FunctionsWithWindowedValue.mapFn(f)).internal,
      context)
