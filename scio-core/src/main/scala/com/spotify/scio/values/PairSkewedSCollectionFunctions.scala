...
private final case class Partitions[K, V](hot: SCollection[(K, V)],
                                          chill: SCollection[(K, V)])
...
class PairSkewedSCollectionFunctions[K, V](val self: SCollection[(K, V)]) {
  ...
  private def partitionInputs[W: Coder](that: SCollection[(K, W)],
                                        ...
                                        cms: SCollection[CMS[K]])(
    implicit koder: Coder[K],
    voder: Coder[V]): (Partitions[K, V], Partitions[K, W]) = {
    ...
    val partitionedSelf = self
      ...
      .transformWithSideOutputs(Seq(hotSelf, chillSelf), "Partition LHS") {
        (e, c) =>
    ...
    val partitionedThat = that
      .withSideInputs(keyCMS, error)
      .transformWithSideOutputs(Seq(hotThat, chillThat), "Partition RHS") {
        (e, c) =>
          if (c(keyCMS).nonEmpty &&
              c(keyCMS).head
                .frequency(e._1)
