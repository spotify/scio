...
class CodersTest extends FlatSpec with Matchers {
  ...
  def checkNotFallback[T: ClassTag](t: T)(implicit C: Coder[T],
                                          eq: Equality[T]): Assertion = {
    C should !==(Coder.kryo[T])
    check[T](t)(C, eq)
  }

  def checkFallback[T: ClassTag](t: T)(implicit C: Coder[T],
                                       eq: Equality[T]): Assertion = {
    C should ===(Coder.kryo[T])
    check[T](t)(C, eq)
  }
  ...
  it should "support Avro's GenericRecord" in {
    ...
    checkNotFallback(record)(classTag[GenericRecord],
