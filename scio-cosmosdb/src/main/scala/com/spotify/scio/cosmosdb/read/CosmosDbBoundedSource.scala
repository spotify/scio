...
private[read] class CosmosDbBoundedSource(private[read] val readCosmos: CosmosDbRead) extends BoundedSource[Document] {

  /** @inheritDoc
   * TODO: You have to find a better way, maybe by partition key */
  override def split(desiredBundleSizeBytes: Long, options: PipelineOptions): util.List[CosmosDbBoundedSource] = Collections.singletonList(this)
