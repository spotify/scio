...
val beamSDKIO =
  Seq(
    "org.apache.beam" % "beam-sdks-java-io-google-cloud-platform" % beamVersion excludeAll (
      ...
      ExclusionRule("com.google.api.grpc",
...
val commonSettings = Sonatype.sonatypeSettings ++ assemblySettings ++ Seq(
  ...
  scmInfo := Some(
    ScmInfo(url("https://github.com/spotify/scio.git"),
...
lazy val root: Project = Project(
  ...
  .settings(
    ...
    unidocProjectFilter in (ScalaUnidoc, unidoc) := inAnyProject
      ...
      -- inProjects(scioRepl) -- inProjects(scioSchemas) -- inProjects(
      scioExamples)
      -- inProjects(scioJmh),
    ...
    unidocAllClasspaths in (ScalaUnidoc, unidoc) := {
      (unidocAllClasspaths in (ScalaUnidoc, unidoc)).value.map { cp =>
        cp.filterNot(_.data.getCanonicalPath.matches(""".*guava-11\..*"""))
          .filterNot(
            _.data.getCanonicalPath.matches(""".*bigtable-client-core-0\..*"""))
      }
    },
...
val javaMappings = beamMappings ++ Seq(
  ...
  ("org.tensorflow",
   "libtensorflow",
   "https://www.tensorflow.org/api_docs/java/reference")
)
