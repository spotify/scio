...
class SimpleKeyGroupReaderTest extends PipelineSpec {
  ...
  it should "correctly handle partial consumption with auto-advance to next key group" in {
...

    {
      ...
      val result = SMBCollection
        .read(SMBKey.primary[Integer], usersRead)
        .flatMapValues(_.headOption) // Partial consumption: takes first, leaves rest
        .toSCollectionAndSeal()
      ...
      val firstNames = result.map { case ((_, _), user) => user.getFirstName.toString }
      ...
      result.map { case ((k, _), _) => k }.distinct should haveSize(3)
