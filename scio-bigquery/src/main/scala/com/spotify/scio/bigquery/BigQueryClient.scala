...
object BigQueryClient {
  ...
  private lazy val instance: BigQueryClient =
    if (sys.props(BigQueryConfig.PROJECT_KEY) != null) {
      BigQueryClient(sys.props(BigQueryConfig.PROJECT_KEY))
    } else {
      ...
      if (project != null) {
        ...
        throw new RuntimeException(
          s"Property ${BigQueryConfig.PROJECT_KEY} not set. " +
            s"Use -D${BigQueryConfig.PROJECT_KEY}=<BILLING_PROJECT>")
      }
    }
  ...
  def apply(project: String): BigQueryClient = {
    val secret = sys.props(BigQueryConfig.SECRET_KEY)
    if (secret == null) {
