...
class MockBigQuery private (private val bq: BigQueryClient) {
   ...
   * data.
   */
  def queryResult(sqlQuery: String,
                  flattenResults: Boolean = false): Seq[TableRow] = {
...
class MockTable(private val bq: BigQueryClient,
   ...
   * Populate the table with mock data.
   */
  def withTypedData[T <: HasAnnotation: ClassTag: TypeTag](
    rows: Seq[T]): Unit = {
    val bqt = BigQueryType[T]
  ...
  def withSample(numRows: Int): Unit = {
    ...
    require(rows.length == numRows,
            s"Sample size ${rows.length} != requested $numRows")
    writeRows(rows)
  }
