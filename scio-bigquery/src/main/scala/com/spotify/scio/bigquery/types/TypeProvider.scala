...
private[types] object TypeProvider {
  ...
  private def schemaToType(c: blackbox.Context)
    ...
    def getRawType(tfs: TableFieldSchema): (Tree, Seq[Tree]) = tfs.getType match {
      ...
      case "TIMESTAMP" => (tq"_root_.java.time.Instant", Nil)
      case "DATE" => (tq"_root_.java.time.LocalDate", Nil)
      case "TIME" => (tq"_root_.java.time.LocalTime", Nil)
      case "DATETIME" => (tq"_root_.java.time.LocalDateTime", Nil)
      case "RECORD" | "STRUCT" =>
