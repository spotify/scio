...
private[types] object TypeProvider {
      ...
      case (clazzDef @ q"$mods class $cName[..$tparams] $ctorMods(..$fields) extends { ..$earlydefns } with ..$parents { $self => ..$body }") :: tail
        ...
        if (parents.map(_.toString()).toSet != Set("scala.Product",
          ...
          c.abort(c.enclosingPosition,
                  s"Invalid annotation, don't extend the case class $clazzDef")
        }
        ...
        val defTblDesc = desc.headOption.map(d =>
          q"override def tableDescription: _root_.java.lang.String = $d")
        ...
        val fnTrait = tq"${TypeName(s"Function${fields.size}")}[..${fields.map(
          _.children.head)}, $cName]"
      ...
      tfs.getType match {
        ...
        case "RECORD" | "STRUCT" =>
          ...
          (q"${Ident(TypeName(name))}",
           Seq(q"case class ${TypeName(name)}(..$fields)") ++ records)
    ...
    val (r, caseClassTree, name) = annottees.map(_.tree) match {
      case (clazzDef @ q"$mods class $cName[..$tparams] $ctorMods(..$cfields) extends { ..$earlydefns } with ..$parents { $self => ..$body }") :: tail
        ...
        if (parents.map(_.toString()).toSet != Set("scala.AnyRef")) {
          c.abort(c.enclosingPosition,
                  s"Invalid annotation, don't extend the class $clazzDef")
        }
        if (cfields.nonEmpty) {
          c.abort(c.enclosingPosition,
                  s"Invalid annotation, don't provide class fields $clazzDef")
        }
  ...
  private def companion(c: blackbox.Context)(
    ...
    val tupled = if (numFields > 1 && numFields <= 22)
      Seq(
