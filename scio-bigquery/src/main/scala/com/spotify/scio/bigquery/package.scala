...
package object bigquery {
  ...
  implicit def toBigQuerySCollection[T](
    c: SCollection[T]): BigQuerySCollection[T] =
    new BigQuerySCollection[T](c)

  /**
   * Enhanced version of [[com.google.api.services.bigquery.model.TableReference TableReference]].
   */
  implicit class RichTableReference(private val r: TableReference)
      extends AnyVal {
    ...
    private[this] val parser = new DateTimeFormatterBuilder()
      ...
      .appendOptional(
        new DateTimeFormatterBuilder()
          .append(null,
                  Array(" ZZZ", "ZZ").map(p =>
                    DateTimeFormat.forPattern(p).getParser))
          .toParser)
      .toFormatter
      .withZoneUTC()
