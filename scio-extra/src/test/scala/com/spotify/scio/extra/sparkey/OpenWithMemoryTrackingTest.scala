...
class OpenWithMemoryTrackingTest extends AnyFlatSpec with Matchers {
  ...
  private def withSparkey(test: File => Unit): Unit = {
  ...
  "openWithMemoryTracking" should "open via mmap when off-heap budget is available" in {
    withSparkey { base =>
      ...
      val reader = ShardedSparkeyUri.openWithMemoryTracking(base, tracker)
      try {
        reader.getAsString("key1") shouldBe "value1"
        reader.getAsString("key2") shouldBe "value2"
      } finally {
        reader.close()
      }
    }
  ...
  it should "fall back to mmap with warning when neither budget is available" in {
    withSparkey { base =>
