...
trait SCollectionSyntax {
  ...
  implicit final class RollupOpsTuple[U, D, R, M](self: SCollection[(U, D, R, M, (Long, Long, Long))]) {
    ...
    def rollupAndCount(
                        rollupFunction: R => Set[R]
                      )(implicit g: Group[M]): SCollection[((D, R), (M, (Long, Long, Long)))] = {
      implicit val (coderU, coderD, coderR, coderM, coderLongTuple) = BeamCoders.getTuple5Coders(self)

      val doubleCounting = self
        .withName("RollupAndCountDuplicates")
        .transform {
          _.map { case (_, dims, rollupDims, measure, measureDistinct) =>
            ((dims, rollupDims), (measure, measureDistinct))
          }.sumByKey
            .flatMap { case (dims @ (_, rollupDims), measure) =>
              rollupFunction(rollupDims)
                .map((x: R) => dims.copy(_2 = x) -> measure)
      ...
      val correctingCounts = self
        ...
        .transform {
          _.map { case (uniqueKey, dims, rollupDims, _, distinctMeasure) =>
            ...
            .filterValues(_.size > 1)
            .flatMapValues { values =>
              val rollupMap = collection.mutable.Map.empty[R, (Long, Long, Long)]
              for ((d, v) <- values)  {
                for (newDim <- rollupFunction(d)) {
                  // Add 1 to correction count. We only care to correct for excessive counts
                  val currentValue = rollupMap.getOrElse(newDim, (1L, 1L, 1L))
                  rollupMap(newDim) = (currentValue._1 - v._1,  currentValue._2 - v._2,  currentValue._3 - v._3)
                }
              }

              rollupMap.iterator
                // We've added 1 initially to not remove the actual count
                // We should have negative numbers, a one means that we never had any counts
                .map{case (d,m) => (d, (min(m._1, 0L), min(m._2, 0L), min(m._3, 0L)))}
                // We only care about correcting cases where we actually double-count
                .filter(_._2 != (0L, 0L, 0L))
            }
            .map { case ((_, dims), (rollupDims, count)) => ((dims, rollupDims), (g.zero, count)) }
        }
