...
package object annoy {
   ...
   */
  class AnnoyReader private[annoy] (path: String,
                                    metric: AnnoyMetric,
                                    dim: Int) {
  ...
  implicit class AnnoyScioContext(val self: ScioContext) extends AnyVal {
     ...
     */
    def annoySideInput(path: String,
                       metric: AnnoyMetric,
                       dim: Int): SideInput[AnnoyReader] = {
  ...
  implicit class AnnoyPairSCollection(
    val self: SCollection[(Int, Array[Float])]) {
     ...
     */
    def asAnnoy(path: String,
                metric: AnnoyMetric,
                dim: Int,
                nTrees: Int): SCollection[AnnoyUri] = {
          ...
          .map {
            case (_, xs) =>
              ...
              try {
                ...
                logger.info(
                  s"Built index with $size items in $elapsedTime seconds")
              } catch {
                case e: Throwable =>
                  annoyWriter.free()
                  throw e
     ...
     * @return A singleton SCollection containing the [[AnnoyUri]] of the saved files
     */
    def asAnnoy(metric: AnnoyMetric,
                dim: Int,
                nTrees: Int): SCollection[AnnoyUri] = {
     ...
     */
    def asAnnoySideInput(metric: AnnoyMetric,
                         dim: Int,
                         nTrees: Int): SideInput[AnnoyReader] =
   ...
   */
  implicit class AnnoySCollection(val self: SCollection[AnnoyUri])
      extends AnyVal {
