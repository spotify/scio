...
package object json extends AutoDerivation {
  ...
  implicit class JsonScioContext(@transient val self: ScioContext)
      extends Serializable {
    def jsonFile[T: ClassTag: Encoder: Decoder: Coder](
  ...
  implicit class JsonSCollection[T: ClassTag: Encoder: Decoder](
    ...
    def saveAsJsonFile(path: String,
      ...
      self.write(JsonIO[T](path))(
        JsonIO.WriteParam(suffix, numShards, compression, printer))
  }
