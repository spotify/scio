...
package object avro {
  ...
  class ParquetAvroFile[T: ClassTag] private[avro] (
     ...
     */
    def flatMap[U: ClassTag: Coder](
      f: T => TraversableOnce[U]): SCollection[U] =
      this
  ...
  object ParquetAvroFile {
    implicit def parquetAvroFileToSCollection[T: Coder](
      self: ParquetAvroFile[T]): SCollection[T] = self.toSCollection
   ...
   * methods.
   */
  implicit class ParquetAvroSCollection[T: ClassTag: Coder](
    ...
    def saveAsParquetAvroFile(path: String,
                              ...
                              compression: CompressionCodecName =
                                CompressionCodecName.SNAPPY): Future[Tap[T]] = {
      val param =
    ...
    def setCredentials(job: Job): Unit = {
      ...
      sys.env.get("GOOGLE_APPLICATION_CREDENTIALS") match {
        ...
        case None =>
          ...
          job.getConfiguration.set("fs.gs.auth.client.id",
