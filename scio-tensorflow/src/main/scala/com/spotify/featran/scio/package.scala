...
package object scio {
  ...
  implicit object ScioCollectionType extends CollectionType[SCollection] {
    override def map[A, B: ClassTag](ma: SCollection[A])(
      f: A => B): SCollection[B] = {
      implicit val coder: Coder[B] = Coder.kryo
      ma.map(f)
    }

    override def reduce[A](ma: SCollection[A])(
      f: (A, A) => A): SCollection[A] = {
    ...
    override def pure[A, B: ClassTag](ma: SCollection[A])(
      b: B): SCollection[B] = {
      implicit val coder: Coder[B] = Coder.kryo
