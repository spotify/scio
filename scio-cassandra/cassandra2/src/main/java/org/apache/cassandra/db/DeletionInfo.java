...
public class DeletionInfo implements IMeasurableMemory {
  ...
  /**
   * This represents a deletion of the entire row. We can't represent this within the
   * RangeTombstoneList, so it's kept separately. This also slightly optimizes the common case of a
   * full row deletion.
   */
  private DeletionTime topLevel;

  /**
   * A list of range tombstones within the row. This is left as null if there are no range
   * tombstones (to save an allocation (since it's a common case).
   */
  ...
  public DeletionInfo(RangeTombstone rangeTombstone, Comparator<Composite> comparator) {
    this(
        ...
        rangeTombstone.max,
        comparator,
        rangeTombstone.data.markedForDeleteAt,
        rangeTombstone.data.localDeletionTime);
  }
  ...
  public DeletionInfo copy(AbstractAllocator allocator) {
    ...
    if (ranges != null) rangesCopy = ranges.copy(allocator);
  ...
  public boolean isDeleted(Cell cell) {
    ...
    if (cell.timestamp() <= topLevel.markedForDeleteAt) return true;

    // No matter what the counter cell's timestamp is, a tombstone always takes precedence. See
    // CASSANDRA-7346.
    if (!topLevel.isLive() && cell instanceof CounterCell) return true;
  ...
  /** Returns a new {@link InOrderTester} given the order in which columns will be passed to it. */
  ...
  public DeletionInfo diff(DeletionInfo superset) {
    RangeTombstoneList rangeDiff =
        superset.ranges == null || superset.ranges.isEmpty()
            ? null
  ...
  public void updateDigest(MessageDigest digest) {
    ...
    if (ranges != null) ranges.updateDigest(digest);
  }
  ...
  /**
   * Potentially replaces the top-level tombstone with another, keeping whichever has the higher
   * markedForDeleteAt timestamp.
   *
   * @param newInfo
   */
  public void add(DeletionTime newInfo) {
    if (topLevel.markedForDeleteAt < newInfo.markedForDeleteAt) topLevel = newInfo;
  }
  ...
  /**
   * Combines another DeletionInfo with this one and returns the result. Whichever top-level
   * tombstone has the higher markedForDeleteAt timestamp will be kept, along with its
   * localDeletionTime. The range tombstones will be combined.
   *
   * @return this object.
   */
  ...
  public Iterator<RangeTombstone> rangeIterator() {
    // FIXME: Guava Iterators.emptyIterator is deprecated, using Java version instead
  ...
  public long unsharedHeapSize() {
    return EMPTY_SIZE
        + topLevel.unsharedHeapSize()
        + (ranges == null ? 0 : ranges.unsharedHeapSize());
  }
  ...
  /**
   * This object allow testing whether a given column (name/timestamp) is deleted or not by this
   * DeletionInfo, assuming that the columns given to this object are passed in forward or reversed
   * comparator sorted order.
   *
   * <p>This is more efficient that calling DeletionInfo.isDeleted() repeatedly in that case.
   */
  public class InOrderTester {
    ...
    public boolean isDeleted(Cell cell) {
       ...
       */
      if (reversed) return DeletionInfo.this.isDeleted(cell);

      // Maybe create the tester if we hadn't yet and we now have some ranges (see above).
      if (tester == null && ranges != null) tester = ranges.inOrderTester();
