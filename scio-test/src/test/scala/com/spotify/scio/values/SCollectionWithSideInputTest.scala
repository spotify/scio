...
class SCollectionWithSideInputTest extends PipelineSpec {
  ...
  it should "support asMultiMapSideInput" in {
    runWithContext { sc =>
      ...
      val s = p1
        ...
        .toSCollection
      s should containInAnyOrder(
        sideData.map(kv => (kv._1, Set(kv._2, kv._2 + 10))))
    }
  }
  ...
  val timestampedData =
    (1 to 100).map(x =>
      (x, new Instant(x * DateTimeConstants.MILLIS_PER_SECOND)))
  ...
  it should "support windowed asIterableSideInput" in {
    runWithContext { sc =>
      ...
      val p2 = sc
        ...
        .asIterableSideInput
      val s = p1.withSideInputs(p2).map((x, s) => (x, s(p2))).toSCollection
      s should forAll[(Int, Iterable[Int])](t =>
        (1 to t._1).toSet == t._2.toSet)
    }
  }
  ...
  it should "support windowed asMapSideInput" in {
    runWithContext { sc =>
      ...
      val p2 = sc
        ...
        .asMapSideInput
      val s = p1.withSideInputs(p2).map((x, s) => (x, s(p2))).toSCollection
      s should forAll[(Int, Map[Int, Int])](t =>
        (1 to t._1).map(_ -> t._1).toMap == t._2)
    }
  }
