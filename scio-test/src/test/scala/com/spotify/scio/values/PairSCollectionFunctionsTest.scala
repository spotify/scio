...
class PairSCollectionFunctionsTest extends PipelineSpec {
  ...
  "PairSCollection" should "support cogroup()" in {
    runWithContext { sc =>
      ...
      val expected = Seq(("a", (iterable(1), iterable(11L))),
  ...
  it should "support cogroup() with duplicate keys" in {
    runWithContext { sc =>
      ...
      val expected =
        Seq[(String, (Set[Int], Set[Long]))](("a", (Set(1, 2), Set(11L))),
                                             ("b", (Set(2), Set(12L, 13L))),
                                             ("c", (Set(3), Set())),
                                             ("d", (Set(), Set(14L))))
      r1 should containInAnyOrder(expected)
  ...
  it should "support leftOuterJoin()" in {
    runWithContext { sc =>
      ...
      p should containInAnyOrder(
        Seq(("a", (1, Some(11))), ("b", (2, Some(12))), ("c", (3, None))))
    }
  }
  ...
  it should "support rightOuterJoin()" in {
    runWithContext { sc =>
      ...
      p should
        containInAnyOrder(
          Seq(("a", (Some(1), 11)), ("b", (Some(2), 12)), ("d", (None, 14))))
    }
  }
  ...
  it should "support approxQuantilesByKey()" in {
    runWithContext { sc =>
      ...
      p should containInAnyOrder(
        Seq(("a", iterable(0, 50, 100)), ("b", iterable(0, 5, 10))))
    }
  }
  ...
  it should "support flatMapValues()" in {
    runWithContext { sc =>
      ...
      p should containInAnyOrder(
        Seq(("a", 11.0), ("a", 21.0), ("b", 12.0), ("b", 22.0)))
    }
  }
  ...
  it should "support sampleByKey()" in {
    runWithContext { sc =>
      val p = sc
        .parallelize(
          Seq(("a", 1), ("b", 2), ("b", 2), ("c", 3), ("c", 3), ("c", 3)))
        .sampleByKey(1)
      p should containInAnyOrder(
        Seq(("a", iterable(1)), ("b", iterable(2)), ("c", iterable(3))))
    }
  }
  ...
  it should "support sumByKey()" in {
    runWithContext { sc =>
      val p = sc
        .parallelize(
          List(("a", 1), ("b", 2), ("b", 2)) ++ (1 to 100).map(("c", _)))
        .sumByKey
  ...
  it should "support topByKey()" in {
    runWithContext { sc =>
      val p = sc.parallelize(
        Seq(("a", 1), ("b", 11), ("b", 12), ("c", 21), ("c", 22), ("c", 23)))
      val r1 = p.topByKey(1)
      val r2 = p.topByKey(1, Ordering.by(-_))
      r1 should
        containInAnyOrder(
          Seq(("a", iterable(1)), ("b", iterable(12)), ("c", iterable(23))))
      r2 should
        containInAnyOrder(
          Seq(("a", iterable(1)), ("b", iterable(11)), ("c", iterable(21))))
    }
  }
