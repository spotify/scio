...
class RichCoderRegistryTest extends PipelineSpec {
  ...
  private def roundTrip[T: ClassTag](value: T) = new Matcher[CoderRegistry] {
    override def apply(left: CoderRegistry): MatchResult = {
      ...
      MatchResult(testRoundTrip(coder, value),
  ...
  it should "support Avro SpecificRecord in joins" in {
    val expected = Seq(new Account(1, "checking", "Alice", 1000.0),
                       new Account(2, "checking", "Bob", 2000.0))

    runWithContext { sc =>
      val lhs = sc
        .parallelize(1 to 10)
        .map(i => new Account(i, "checking", "u" + i, i * 1000.0))
      ...
      lhs
        ...
        .mapValues {
          case (account, name) =>
            Account.newBuilder(account).setName(name).build()
        }
  ...
  it should "give ProtoCoder higher precedence" in {
    ...
    registry.getScalaCoder[Timestamp](options) shouldBe ProtoCoder.of(
      classOf[Timestamp])
  }
