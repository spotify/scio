...
class DynamicFileTest extends PipelineSpec {
  ...
  "Dynamic File" should "support text files" in {
    ...
    lines0 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 0).map(_.toString))
    lines1 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 1).map(_.toString))
    sc2.close()
  ...
  it should "support generic Avro files" in {
    ...
    lines0 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 0).map(newGenericRecord))
    lines1 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 1).map(newGenericRecord))
    sc2.close()
    FileUtils.deleteDirectory(tmpDir.toFile)
  }
  ...
  it should "support specific Avro files" in {
    ...
    lines0 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 0).map(newSpecificRecord))
    lines1 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 1).map(newSpecificRecord))
    sc2.close()
    FileUtils.deleteDirectory(tmpDir.toFile)
  }
  ...
  it should "support Avro files with windowing" in {
    ...
    sc1
      ...
      .withFixedWindows(Duration.standardMinutes(1),
                        Duration.ZERO,
                        WindowOptions())
      .saveAsDynamicAvroFile(tmpDir.toString, 1, schema)(r =>
        (r.getIntField % 2).toString)
    sc1.close()
    verifyOutput(tmpDir, "0", "1")
    Files.list(tmpDir.resolve("0")).iterator().asScala.size shouldBe 5
    ...
    records0 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 0).map(newSpecificRecord))
    records1 should containInAnyOrder(
      (1 to 10).filter(_ % 2 == 1).map(newSpecificRecord))
    (1 to 10).foreach { x =>
      val p = x % 2
      val t1 = new Instant(x * 60000)
