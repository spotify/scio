...
trait SCollectionMatchers {
  ...
  private def makeFn[T](
    f: JIterable[T] => Unit): SerializableFunction[JIterable[T], Void] =
  ...
  private def serDeCycle[T: Coder](
    scollection: SCollection[T]): SCollection[T] = {
  ...
  def containInAnyOrder[T: Coder](
    value: Iterable[T]): IterableMatcher[SCollection[T], T] =
      ...
      override def matcher(builder: AssertBuilder): Matcher[SCollection[T]] =
        new Matcher[SCollection[T]] {
          override def apply(left: SCollection[T]): MatchResult = {
            ...
            m(() =>
                builder(
                  PAssert.that(serDeCycle(left).internal).satisfies(should)),
              () =>
                builder(
                  PAssert.that(serDeCycle(left).internal).satisfies(shouldNot)))
          }
        }
    }
  ...
  def equalMapOf[K: Coder, V: Coder](
    ...
    new SingleMatcher[SCollection[(K, V)], JMap[K, V]] {
      override def matcher(
        builder: AssertBuilder): Matcher[SCollection[(K, V)]] =
  ...
  def satisfy[T: Coder](
    predicate: Iterable[T] => Boolean): IterableMatcher[SCollection[T], T] =
    new IterableMatcher[SCollection[T], T] {
      override def matcher(builder: AssertBuilder): Matcher[SCollection[T]] =
        new Matcher[SCollection[T]] {
          override def apply(left: SCollection[T]): MatchResult = {
  ...
  def satisfySingleValue[T: Coder](
    predicate: T => Boolean): SingleMatcher[SCollection[T], T] =
    new SingleMatcher[SCollection[T], T] {
      override def matcher(builder: AssertBuilder): Matcher[SCollection[T]] =
        new Matcher[SCollection[T]] {
          override def apply(left: SCollection[T]): MatchResult = {
  ...
  def forAll[T: Coder](
    predicate: T => Boolean): IterableMatcher[SCollection[T], T] = {
