...
object IsJavaBean {
  transparent inline implicit def isJavaBean[T]: IsJavaBean[T] = ${IsJavaBean.isJavaBeanImpl[T]}

  def apply[T](implicit i: IsJavaBean[T]): IsJavaBean[T] = i

   // TODO migration: not yet implemented 
  private def checkGetterAndSetters(using Quotes)(t: quotes.reflect.TypeRepr): Unit = {
    // val getters =
    //   t.decls.collect {
  ...
  def isJavaBeanImpl[T: Type](using Quotes): Expr[IsJavaBean[T]] = {
    import quotes.reflect._
    val wtt = TypeRepr.of[T]
    val sym = wtt.typeSymbol
    val isJava = sym.flags.is(Flags.JavaDefined)
    val isClass = sym.isClassDef
    if (isJava && isClass) {
      checkGetterAndSetters(wtt)
      '{null: _root_.com.spotify.scio.IsJavaBean[T]}
    } else {
      report.errorAndAbort(
        s"${wtt.show} is not a Java class. (isJava: ${isJava}, isClass: ${isClass})"
      )
    }
  }
}
