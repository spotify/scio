...
final class GenericRecordSCollectionOps(private val self: SCollection[GenericRecord])
  ...
  def saveAsAvroFile(
    ...
    useReflectApi: Boolean = AvroIO.WriteParam.DefaultUseReflectApi
  ): ClosedTap[GenericRecord] = {
    val param = AvroIO.WriteParam(numShards, suffix, codec, metadata, tempDirectory, useReflectApi)
...
final class ObjectFileSCollectionOps[T](private val self: SCollection[T]) extends AnyVal {
  ...
  def saveAsObjectFile(
    ...
    val param =
      ObjectFileIO.WriteParam(numShards, suffix, codec, metadata, tempDirectory, useReflectApi)
