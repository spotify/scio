...
private[types] object TypeProvider {
  ...
  def schemaFileImpl(c: blackbox.Context)(
  ...
  private def readFromFileSystem(file: String): InputStream = {
    ...
    Channels.newInputStream(
      FileSystems.open(files.metadata().get(0).resourceId()))
  }
  ...
  def toSchemaImpl(c: blackbox.Context)(
    annottees: c.Expr[Any]*): c.Expr[Any] = {
    import c.universe._
    ...
    val r = annottees.map(_.tree) match {
      case l @ List(
        ...
        if (parents.map(_.toString()).toSet != Set("scala.Product",
                                                   "scala.Serializable")) {
          c.abort(c.enclosingPosition,
                  s"Invalid annotation, don't extend the case class $l")
        }
        ...
        val fnTrait = if (fields.size <= 22) {
          Seq(tq"${TypeName(s"Function${fields.size}")}[..${fields.map(
            _.children.head)}, $name]")
        } else {
          Seq()
        }
  ...
  private def schemaToType(c: blackbox.Context)(
    ...
    def getField(className: String,
      ...
      fieldSchema.getType match {
        ...
        case MAP =>
          ...
          (tq"_root_.scala.collection.Map[_root_.java.lang.String,$fieldType]",
           recordCaseClasses)
        case RECORD =>
    ...
    def extractField(className: String,
      ...
      val (fieldType, recordClasses) =
        getField(className,
                 SchemaUtil.unescapeNameIfReserved(fieldName),
                 fieldSchema)
      fieldSchema.getType match {
        case UNION =>
    ...
    def extractFields(className: String,
                      schema: Schema): (Seq[Tree], Seq[Tree]) = {
    ...
    val r = annottees.map(_.tree) match {
      case l @ List(
        ...
        if (cfields.nonEmpty) {
          c.abort(c.enclosingPosition,
                  s"Invalid annotation, don't provide class fields $l")
        }
        ...
        if (shouldDumpClassesForPlugin) {
          dumpCodeForScalaPlugin(c)(recordClasses,
                                    caseClassTree,
                                    name.toString())
        }
  ...
  private def companion(c: blackbox.Context)(name: c.TypeName,
    ...
    val tupledMethod =
      if (fields.size > 1 && fields.size <= 22) {
        ...
        Seq(
          q"$overrideFlag def tupled = (${TermName(name.toString)}.apply _).tupled")
      } else {
        Seq()
      }
  ...
  private def extractStrings(c: blackbox.Context,
                             errorMessage: String): List[String] = {
    import c.universe._
  ...
  private def dumpCodeForScalaPlugin(c: blackbox.Context)(
    records: Seq[c.universe.Tree],
    ...
    logger.debug(
      s"Will dump generated $name of $owner from $srcFile to $genSrcFile")
