...
object AutoComplete {
  ...
  def computeTopCompletions(
    ...
    if (recursive) {
      SCollection.unionAll(
        computeTopRecursive(candidates, candidatesPerPrefix, 1))
    } else {
  ...
  def makeEntity(kind: String,
                 kv: (String, Iterable[(String, Long)])): Entity = {
    ...
    val candidates = kv._2.map { p =>
      makeValue(
        Entity
          ...
          .putAllProperties(
            ImmutableMap.of("tag",
                            makeValue(p._1).build(),
                            "count",
                            makeValue(p._2).build()))).build()
    }
    Entity
      .newBuilder()
      .setKey(key)
      .putAllProperties(
        ImmutableMap.of("candidates", makeValue(candidates.asJava).build()))
      .build()
  }
